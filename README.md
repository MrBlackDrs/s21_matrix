...
В данном проекте реализована библиотека **s21_matrix** для работы с матрицами, используя **объектно-ориентированный подход**. ООП позволяет реализовать библиотеку для работы с матрицами в виде отдельного класса, над объектами которого определены операции, представленные как методами, так и стандартными операторами +, -, * и т.д.
...
Ниже представлено краткое описание операций над матрицами, реализованные в разработанной библиотеке. 
...
- В приватных полях хранится только `matrix_`, `rows_` и `cols_`;
...
- Цель gcov_report готовит отчет покрытия тестами
![s21_matrix](misc/images/gcov_report.PNG)

## Sample

- Пример использования библиотеки
- В папке src запускаем цель ``` make sample``` и вводим две матрицы
![s21_matrix](misc/images/application.PNG)
